  ...
  template: `
    @if (gameData$ | async; as gameDetails) {
      <div class="p-8 bg-white shadow dark:bg-gray-800 relative">
        <p
          class="mb-6 text-xl font-normal text-center text-gray-500 dark:text-gray-300"
        >
        ...
        <p
          ...
          [ngClass]="{
            'text-green-500 dark:text-green-500':
              gameDetails[0].status === 'started',
            'text-red-500 dark:text-red-500':
              gameDetails[0].status === 'closed',
          }"
        >
        ...
        <div class="flex justify-center mb-8">
          @if (gameDetails[0].status === 'started') {
            <span
          ...
          @if (gameDetails[0].status === 'closed') {
            <span
        ...
        />
        <p
          class="mb-12 text-xl font-normal text-center text-gray-500 dark:text-gray-300"
        >
        ...
        @if (players$ | async; as players) {
          ...
          >
            @for (item of players; track item; let first = $first) {
              <div
                class="relative p-4"
                [ngClass]="{
                  'new-box': first && gameDetails[0].status === 'ready',
                }"
              >
                @if (item.win) {
                  <div
                ...
                >
                    ...
                    <a href="#" class="relative block">
                      <img
                        ...
                        src="https://ui-avatars.com/api/?name={{
                          item.name
                        }}&size=150&background={{ item.ticketID }}"
                  ...
                  <div class="mt-2 text-center flex flex-col">
                    ...
                    <span
                      class="rounded-full text-white px-3 py-1 text-xs uppercase font-medium bg-slate-400"
                      >{{ item.ticketID }}</span
                    >
                  </div>
                </div>
